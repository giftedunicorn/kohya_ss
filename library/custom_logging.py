...
def setup_logging(clean=False, debug=False):
    ...
    #     logging.basicConfig(level=logging.DEBUG, format='%(asctime)s | %(levelname)s | %(pathname)s | %(message)s', filemode='a', encoding='utf-8', force=True)
    # else:
    #     logging.basicConfig(level=logging.DEBUG, format='%(asctime)s | %(levelname)s | %(pathname)s | %(message)s', filemode='a', force=True)
